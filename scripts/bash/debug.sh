...
export TRAINING_DATASET_JSON=$SCRIPT_DIR/../../data_recipe/debug.yaml

export PRETRAIN_NAME=Qwen2-0.5B-Siglip-Pretrain-Qwen2-0.5B-Stage2-checkpoint-2-continue
export TRAINING_NAME=debug

export NUM_TRAIN_EPOCHS=3

mkdir -p $SCRIPT_DIR/../../playground/training/training_dirs/$TRAINING_NAME

ACCELERATE_CPU_AFFINITY=1 torchrun --nnodes $NNODES --nproc_per_node $GPUS_PER_NODE --node_rank $SLURM_NODEID --master_addr $(scontrol show hostname $SLURM_NODELIST | head -n1) --master_port ${MASTER_PORT} $SCRIPT_DIR/../../llava/train/train_mem.py \
