...
class ModelWorker:
    ...
    def generate_stream(self, params):
        ...
        if images is not None and len(images) > 0:
            if len(images) > 0:
                if len(images) != prompt.count(DEFAULT_IMAGE_TOKEN):
                    raise ValueError("Number of images does not match number of <Mineru-Image> tokens in prompt")
        ...
        max_context_length = getattr(model.config, 'max_position_embeddings', 32768)
        max_new_tokens = min(int(params.get("max_new_tokens", 16384)), 16384)
...
async def generate_stream(request: Request):
    logger.info(f"generate_stream. Request: {request}")
